...
const TransactionLayout = ({onToggle,isOpen}:TransactionLayoutProps) => {
  ...
  const categoriesData = useAppSelector((state) => state.transaction.categories)
  const categories = useMemo(
    () => [...new Set(categoriesData.map((c) => c.name))] as TransactionType['category'][],
    [data],
  )
           ...
           <div className='container-main space-y-6'>
            <section className='rounded-2xl border border-slate-700 bg-linear-to-r from-slate-800 to-slate-900 p-4 md:p-6 shadow-lg'>
              <div className='flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between'>
                <div>
                  <p className='text-xs tracking-wider text-slate-400'>TRANSACTIONS</p>
                  <h1 className='mt-2 text-2xl md:text-3xl font-bold text-slate-100'>Browse and inspect spending</h1>
                  <p className='mt-2 max-w-2xl text-sm md:text-base text-slate-300'>
                    Filter by category or date range to quickly find a transaction, then compare it against your broader spending trend.
                  </p>
                </div>
                ...
                <div className='grid grid-cols-2 gap-3 sm:grid-cols-4'>
                  <div className='rounded-xl border border-slate-700 bg-slate-800/70 px-3 py-2'>
                    ...
                    <p className='mt-1 text-lg font-semibold text-slate-100'>{selectedCount}</p>
                  </div>
                  <div className='rounded-xl border border-slate-700 bg-slate-800/70 px-3 py-2'>
                    <p className='text-[11px] uppercase tracking-wide text-slate-400'>Categories</p>
                    <p className='mt-1 text-lg font-semibold text-slate-100'>{categories.length}</p>
                  </div>
                  <div className='rounded-xl border border-slate-700 bg-slate-800/70 px-3 py-2'>
                    <p className='text-[11px] uppercase tracking-wide text-slate-400'>Date filter</p>
                    <p className='mt-1 text-sm font-medium text-slate-100'>{dateFrom || 'Start'} → {dateTo || 'End'}</p>
                  </div>
                  <div className='rounded-xl border border-slate-700 bg-slate-800/70 px-3 py-2'>
                    <p className='text-[11px] uppercase tracking-wide text-slate-400'>Focus</p>
                    <p className='mt-1 text-sm font-medium text-slate-100'>{query ?? 'All categories'}</p>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section className='grid grid-cols-1 gap-4 xl:grid-cols-[1.15fr_0.85fr]'>
              <div className='space-y-4 rounded-2xl border border-slate-700 bg-slate-800/70 p-4 md:p-5 shadow-lg'>
                <div className='flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between'>
                  <div>
                    <h2 className='text-lg font-semibold text-slate-100'>Transaction list</h2>
                    <p className='text-sm text-slate-400'>Use the filter bar to narrow down the records below.</p>
                  </div>

                  <div className='flex flex-wrap items-center gap-2'>
                    <select
                      value={query ?? ''}
                      onChange={(e) => setQuery(e.target.value ? (e.target.value as TransactionType['category']) : undefined)}
              ...
              <div className='space-y-4'>
                ...
                <div className='rounded-2xl border border-slate-700 bg-slate-800/70 p-4 md:p-5 shadow-lg'>
                  <div className='mb-3 flex items-center justify-between'>
                    <div>
                      <h2 className='text-lg font-semibold text-slate-100'>Spending trend</h2>
                      <p className='text-sm text-slate-400'>Monthly comparison of your recorded expenses.</p>
                    </div>
                  </div>
