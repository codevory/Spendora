...
const UserAccountPage = ({onToggle,isLoggedin,isOpen}:UserAccountPropsType) => {
const navigate = useNavigate()
const [data,setData] = useState<userData | null>(null)
const [isLoading, setIsLoading] = useState<boolean>(true)
const [error, setError] = useState<string | null>(null)
const transactions = useAppSelector((state) => state.transaction.transactions)
const incomes = useAppSelector((state) => state.incomeTransaction.incomeTransactions)
...
useEffect(() => {
  ...
  const loadUser = async () => {
    try {
      ...
      const docData = snapshot.docs[0]?.data() as Partial<userData> | undefined
...
const handleDeleteAccount = () => {
  setError('Account deletion requires re-authentication. This action is not enabled yet.')
}
      ...
      <div className='bg-main min-h-[calc(100vh-4rem)] p-4 md:p-6'>
        ...
        <div className='space-y-6'>
          <section className='rounded-2xl border border-slate-700 bg-linear-to-r from-slate-800 to-slate-900 p-4 md:p-6 shadow-lg'>
            ...
            <h1 className='mt-2 text-2xl md:text-3xl font-bold text-slate-100'>Profile and preferences</h1>
            <p className='mt-2 max-w-2xl text-sm md:text-base text-slate-300'>
              Manage your personal details and quickly review your Spendora account performance.
            </p>
            {error ? (
          ...
          <section className='grid grid-cols-1 gap-4 xl:grid-cols-[1fr_1fr]'>
            ...
            <div className='rounded-2xl border border-slate-700 bg-slate-800/70 p-5 shadow-lg'>
              <h2 className='text-xl font-semibold text-slate-100'>Financial summary</h2>
              <p className='mt-1 text-sm text-slate-400'>A quick snapshot of your account activity.</p>

              <div className='mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2'>
                <div className='rounded-xl border border-slate-700 bg-slate-900/70 p-3'>
                  <p className='text-xs text-slate-400'>Total income</p>
                  <p className='mt-1 text-xl font-bold text-emerald-300'>INR {accountStats.totalIncome.toLocaleString('en-IN')}</p>
                </div>
                <div className='rounded-xl border border-slate-700 bg-slate-900/70 p-3'>
                  <p className='text-xs text-slate-400'>Total expense</p>
                  <p className='mt-1 text-xl font-bold text-rose-300'>INR {accountStats.totalExpense.toLocaleString('en-IN')}</p>
                </div>
                <div className='rounded-xl border border-slate-700 bg-slate-900/70 p-3'>
                  <p className='text-xs text-slate-400'>Current balance</p>
                  <p className={`mt-1 text-xl font-bold ${accountStats.balance >= 0 ? 'text-emerald-300' : 'text-rose-300'}`}>
                    INR {accountStats.balance.toLocaleString('en-IN')}
                  </p>
                </div>
                <div className='rounded-xl border border-slate-700 bg-slate-900/70 p-3'>
                  <p className='text-xs text-slate-400'>Transactions</p>
                  <p className='mt-1 text-xl font-bold text-slate-100'>{accountStats.transactionCount}</p>
                </div>
              </div>
            </div>
          </section>
