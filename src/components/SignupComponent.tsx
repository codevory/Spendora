            ...
            <div className='flex flex-col gap-3'>
               <div>
                   ...
                   <div className='relative input flex gap-1 items-center'>
                       <input className='input-box' id='username' minLength={6} placeholder='John' value={username} onChange={(e) => setUsername(e.target.value)} required/>
               ...
               <div>
                ...
                <div>
                   ...
                   <div className='relative input input-feld'>
                     <input className='input-box ' id='email' minLength={12} type='email' value={email} onChange={(e) => setEmail(e.target.value)} required/>
                     <p className='absolute bottom-3 left-1 '>{<BiLock size={22}/> }</p>
                   </div>
               </div>
            </div>
               <button className='btn-primary' type='submit'>{isLoading ? <Loader /> : 'Signup'}</button>
               <Link to='/signin' className='absolute top-1/1 right-0  w-23 h-auto items-center rounded-md login-btn '>sign in</Link>
           </form>
