    ...
    <div className='sidebar flex h-full flex-col gap-3 py-3'>
      {navItems.map((item) => {
        ...
        return (
          ...
          >
            <Icon size={20} />
            {isOpen ? <span className='text-sm font-semibold'>{item.label}</span> : null}
          </NavLink>
        );
      })}

      <div className={`mt-2 rounded-xl border border-slate-700 bg-slate-800/70 px-3 py-2 ${isOpen ? '' : 'hidden'}`}>
        <p className='text-[11px] uppercase tracking-[0.2em] text-slate-500'>Coming soon</p>
