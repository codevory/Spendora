...
const DisplayAvailableCategories = ({data}:DisplayCategoriesPropsType) => {
    ...
    const confirmDelete = (val:string) => window.confirm(`are you sure to delete ${val} category`)
  ...
  return (
    <div className='space-y-3'>
      ...
      {categories.map((txn) => {
        const usageCount = data.filter((item) => item.category === txn.name).length

        return (
          <article key={txn.id} className='rounded-xl border border-slate-700 bg-slate-900/80 p-4 shadow-md transition hover:border-slate-500'>
            <div className='flex items-start justify-between gap-3'>
              <div>
                <p className='text-xs uppercase tracking-[0.25em] text-slate-400'>Category</p>
                <h3 className='mt-1 text-lg font-semibold text-slate-100'>{txn.name}</h3>
              </div>
            ...
            <div className='mt-4 flex items-center justify-between gap-3'>
              <p className='text-sm text-slate-400'>Used in your transaction history</p>
              <button
