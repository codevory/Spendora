        ...
        const noData = (
            <div className='card glass flex min-h-80 flex-col items-center justify-center gap-4 border border-slate-700 text-center'>
                ...
                <div className='space-y-1'>
                    <h2 className='text-xl font-semibold text-slate-100'>Nothing matches the current filters</h2>
                    <p className='text-sm text-slate-400'>
                        Try a different category or expand the date range to reveal more results.
                    </p>
                </div>
            </div>
        )
        ...
        <div className='transaction-container max-h-dvh overflow-y-auto rounded-2xl border border-slate-700 bg-slate-900/70 p-3 shadow-lg'>
                <div className='mb-3 flex items-center justify-between px-1'>
                        <div>
                            ...
                            <p className='text-xs text-slate-400'>Showing {visibleData.length} records</p>
                        </div>
                ...
                <div className='space-y-2 pt-3'>
                        {sortedData.map((txn,idx) => (
                            <Link key={txn.transactionId} to={`/transactions/tnx-details/${txn.transactionId}`} className='block rounded-xl border border-slate-700 bg-slate-800/70 transition hover:border-slate-500 hover:bg-slate-800'>
                                <div className='grid grid-cols-[0.45fr_1.45fr_0.7fr] items-center gap-3 px-3 py-3'>
                                    ...
                                    <div className='min-w-0'>
                                        <p className='truncate font-medium text-slate-100'>{txn.name}</p>
                                        <p className='truncate text-xs text-slate-400'>
                                            {txn.transactionId.slice(0, 12)} • {formatTransactionDate(txn.date)}
                                        </p>
                                    </div>
