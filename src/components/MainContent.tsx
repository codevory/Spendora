      ...
      <div className='container-main space-y-6'>
        ...
        <section className='grid grid-cols-1 gap-4 xl:grid-cols-[1.4fr_0.9fr_0.7fr]'>
          <div className='rounded-2xl border border-slate-700 bg-slate-800/70 p-4 md:p-5 shadow-lg xl:col-span-2'>
            <div className='mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between'>
              <div>
                <h2 className='text-xl font-semibold text-slate-100'>{activeGraphTitle}</h2>
                <p className='mt-1 text-sm text-slate-400'>{activeGraphDescription}</p>
              </div>
          ...
          <div className='space-y-4'>
            <div className='h-auto min-w-0 w-full rounded-2xl border border-slate-700 bg-slate-800/70 p-4 shadow-lg md:p-5'>
              <h2 className='mb-3 text-lg font-semibold text-slate-100'>Add transaction</h2>
            ...
            <div className='rounded-2xl border border-slate-700 bg-linear-to-br from-slate-800 to-slate-900 p-4 shadow-lg'>
              <p className='text-xs tracking-[0.22em] text-slate-400'>7 DAY SNAPSHOT</p>
              <h3 className='mt-2 text-lg font-semibold text-slate-100'>Weekly spending pulse</h3>

              <div className='mt-3 grid grid-cols-1 gap-3'>
                <div className='rounded-xl border border-slate-700 bg-slate-800/70 p-3'>
                  <p className='text-xs text-slate-400'>Total spent</p>
                  <p className='mt-1 text-xl font-bold text-rose-300'>₹ {weeklySnapshot.totalSpent.toLocaleString('en-IN')}</p>
                </div>
                <div className='rounded-xl border border-slate-700 bg-slate-800/70 p-3'>
                  <p className='text-xs text-slate-400'>Transactions</p>
                  <p className='mt-1 text-xl font-bold text-slate-100'>{weeklySnapshot.count}</p>
                </div>
                <div className='rounded-xl border border-slate-700 bg-slate-800/70 p-3'>
                  <p className='text-xs text-slate-400'>Top category</p>
                  <p className='mt-1 text-base font-semibold text-indigo-300'>
                    {weeklySnapshot.topCategory ? weeklySnapshot.topCategory[0] : 'No data yet'}
                  </p>
                </div>
              </div>
            </div>
        ...
        <section className='rounded-2xl border border-slate-700 bg-slate-800/70 p-4 md:p-5 shadow-lg'>
          <div className='mb-3'>
            ...
            <p className='text-sm text-slate-400'>Latest debits and credits in chronological order.</p>
          </div>
